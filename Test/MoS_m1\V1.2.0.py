...
class MouseSteeringApp:
    ...
    def change_theme(self, selected_theme):
        if selected_theme == "Dark":
            ...
            self.start_button.configure(bg="#27AE60", fg="white")
            self.stop_button.configure(bg="#E74C3C", fg="white")
            ...
            for widget in self.indicator_frame.winfo_children():
                widget.configure(bg="#ECF0F1", fg="black")

        elif selected_theme == "Light":
            self.root.configure(bg="#ECF0F1")
            self.start_button.configure(bg="#27AE60", fg="black")
            self.stop_button.configure(bg="#E74C3C", fg="black")
            self.sensitivity_slider.configure(bg="#FFFFFF", fg="black", troughcolor="#BDC3C7")
            self.sensitivity_label.configure(bg="#ECF0F1", fg="black")
            self.angle_label.configure(bg="#ECF0F1", fg="black")
            self.offset_label.configure(bg="#ECF0F1", fg="black")
            for widget in self.indicator_frame.winfo_children():
                widget.configure(bg="#ECF0F1", fg="black")
        else:  # Default theme
            self.root.configure(bg="#2C3E50")
    ...
    def on_close(self):
        self.stop_steering()  # Ensure the steering is stopped
        if self.mouse_listener is not None:
            self.mouse_listener.stop()
        self.root.destroy()
